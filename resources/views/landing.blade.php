...
<body>
    ...
    <h4 class="logo-brand"><span class="text-light">ARHZ</span><span class="text-warning">STORE</span></h4>
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
                <li class="nav-item">
                    <a class="btn btn-outline-light me-1" href="{{route('cart.list')}}">
                        <i class="bi-cart-fill me-1"></i>
                        Cart
                        <span class="badge bg-light text-dark ms-1 rounded-pill">{{$qty}}</span>
                    </a>
                </li>
                @auth
                    <li class="nav-item">
    ...
    @forelse ($products->where('status', 'approve') as $product)
        <!-- Product card -->
       <!-- Product card -->
        <form class="col mb-3" action="{{ route('cart.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
